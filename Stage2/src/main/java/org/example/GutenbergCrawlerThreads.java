...
public class GutenbergCrawlerThreads {
    ...
    private static final BlockingQueue<Integer> queue = new LinkedBlockingQueue<>();  // Cola para pasar los libros
    private static final ExecutorService executorService = Executors.newFixedThreadPool(4);  // Pool de 4 hilos consumidores
    ...
    public static void main(String[] args) {
        ...
        // Crear los hilos consumidores que procesarán los libros
        for (int i = 0; i < 4; i++) { // Creamos 4 hilos consumidores
            executorService.submit(new BookProcessor());
        }

        // Descargar los libros y agregar sus IDs a la cola
        for (int i = startId; i < startId + NUM_BOOKS; i++) {
            try {
                ...
                // Añadir el ID del libro a la cola para ser procesado
                queue.put(i);
            } catch (IOException | InterruptedException e) {
                System.out.println("Error al procesar el libro con ID: " + i);
            }
        }
    ...
    // Clase BookProcessor como un consumidor que procesa los libros
    static class BookProcessor implements Runnable {
        @Override
        public void run() {
            while (true) {
                try {
                    // Obtener el ID del libro desde la cola
                    Integer bookId = queue.take();
                    processingBook(bookId);  // Procesar el libro
